...
public class SkinAnalysisResultActivity extends Activity {
					...
					public void run() {
						...
						try {
							...
							final AlertDialog dialog = (new AlertDialog.Builder(
									SkinAnalysisResultActivity.this)).create();
							dialog.show();
	...
	public void onCreate(Bundle bundle) {
		...
		addtoReportBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				String getUserPin = PreferencesUtils
						.getStringFromPreferences(
								SkinAnalysisResultActivity.this,
								ConstHttpProp.USER_PIN);
		...
		case SkinAnalysisActivity.MSG_COLLAGEN:
			...
			break;
		case SkinTouchView.ANALYSIS_MODE_PORE:
			...
			break;
		case SkinTouchView.ANALYSIS_MODE_ACNE:
			skanContentText = getResources().getString(R.string.acne_radius);
			sentContent = "1";
			skanContent = String.format("%.2f", getIntent().getExtras()
					.getFloat("content"))
					+ "mm";
			break;
		case SkinTouchView.ANALYSIS_MODE_SENSI_NUM:
			skanContentText = getResources().getString(
					R.string.blood_streak_num);
			sentContent = "1";
			skanContent = getIntent().getExtras().getInt("content") + "";
			break;
		case SkinTouchView.ANALYSIS_MODE_SENSI_AREA:
			analysisMode = SkinTouchView.ANALYSIS_MODE_SENSI_NUM;
			skanContentText = getResources().getString(
					R.string.blood_streak_area);
			sentContent = "1";
			skanContent = String.format("%.2f", getIntent().getExtras()
					.getFloat("content"))
					+ "mm^2";
		...
		httpsetting.setListener(new HttpGroup.OnAllListener() {
			...
			public void onEnd(HttpResponse response) {
				...
				if (response.getJSONObject() != null) {
					try {
						jsonPoxy = response.getJSONObject().getJSONArray(
								"skinInfo");
	...
	private void addToServer(String currentDate, String userPin, int type,
			String content) {
		JSONObject jsonobject = new JSONObject();
		try {
